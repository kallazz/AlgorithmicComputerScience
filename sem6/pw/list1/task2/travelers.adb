...
-- Travelers moving on the board
  ...
  Nr_Of_Travelers : constant Integer := 25;
  ...
  Deadlock_Timeout : constant Duration := 5.0 * Max_Delay;
...
-- Types, procedures and functions
  ...
  task body Traveler_Task_Type is
    ...
    -- Try to make a step in the given direction with a non-blocking approach
    procedure Try_Direction(Direction : Integer; Success : out Boolean) is
    ...
    begin
      ...
      case Direction is
        ...
        when others =>
          Put_Line("Invalid direction: " & Integer'Image(Direction));
          return;
      end case;
    ...
    begin
      ...
      while not Success loop
        ...
        Direction := Integer(Float'Floor(4.0 * Random(G)));
        
        -- Try to move in that direction
        Try_Direction(Direction, Success);
  ...
  begin
    accept Init(Id: Integer; Seed: Integer; Symbol: Character) do
      ...
      -- Find an unoccupied initial position
      Find_Position: loop
        Traveler.Position := (
        ...
        -- Try to acquire this position
        select
          Board(Traveler.Position.X, Traveler.Position.Y).Acquire(Traveler.Id);
          exit Find_Position; -- Successfully acquired
        else
          null; -- Try another position
        end select;
